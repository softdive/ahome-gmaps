...
public class GMapsLibraryLoader {
	 ...
	 * >Libraries API Doc</a>
	 */
	public static enum GmapLibrary {
		...
		public static GmapLibrary fromValue(String value) {
	...
	public static enum Language {
		...
		GERMAN("de"), GREEK("el"), ENGLISH("en"), ENGLISH_AUSTRALIAN("en-AU"), ENGLISH_GREAT_BRITAIN(
				"en-GB"), SPANISH("es"), BASQUE("eu"), FARSI("fa"), FINNISH(
				"fi"), FILIPINO("fil"), FRENCH("fr"), GALICIAN("gl"), GUJARATI(
				"gu"), HINDI("hi"), CROATIAN("hr"), HUNGARIAN("hu"), INDONESIAN(
				"id"), ITALIAN("it"), HEBREW("iw"), JAPANESE("ja"), KANNADA(
				"kn"), KOREAN("ko"), LITHUANIAN("lt"), LATVIAN("lv"), MALAYALAM(
				"ml"), MARATHI("mr"), DUTCH("nl"), NORWEGIAN_NYNORSK("nn"), NORWEGIAN(
				"no"), ORIYA("or"), POLISH("pl"), PORTUGUESE("pt"), PORTUGUESE_BRAZIL(
				"pt-BR"), PORTUGUESE_PORTUGAL("pt-PT"), ROMANSCH("rm"), ROMANIAN(
				"ro"), RUSSIAN("ru"), SLOVAK("sk"), SLOVENIAN("sl"), SERBIAN(
				"sr"), SWEDISH("sv"), TAGALOG("tl"), TAMIL("ta"), TELUGU("te"), THAI(
				"th"), TURKISH("tr"), UKRAINIAN("uk"), VIETNAMESE("vi"), CHINESE_SIMPLIFIED(
				"zh-CN"), CHINESE_TRADITIONAL("zh-TW"), DANISH("da");
	 ...
	 */
	public static void load(Runnable onLoad, boolean sensor) {
	...
	public static void load(Runnable onLoad,
			ArrayList<GmapLibrary> loadLibraries, boolean sensor) {
	 ...
	 *            derive location [true|false]
	 */
	public static void load(Runnable onLoad, GmapLibrary[] loadLibraries,
			boolean sensor) {
		ArrayList<GmapLibrary> loadLibrariesList = new ArrayList<GmapLibrary>(
	 ...
	 */
	public static void load(Runnable onLoad, GmapLibrary[] loadLibraries,
			boolean sensor, Language language) {
		ArrayList<GmapLibrary> loadLibrariesList = new ArrayList<GmapLibrary>(
				Arrays.asList(loadLibraries));
	 ...
	 */
	public static void load(Runnable onLoad, boolean sensor, String otherParams) {
	 ...
	 *            add additional params. like "key=YOUR_API_KEY"
	 */
	public static void load(final Runnable onLoad,
			ArrayList<GmapLibrary> loadLibraries, boolean sensor,
			Language language, String otherParams) {
		load(onLoad, sensor, loadLibraries, language, otherParams);
	}

	private static void load(final Runnable onLoad, boolean sensor,
			ArrayList<GmapLibrary> loadLibraries, Language language,
			String otherParams) {
	 ...
	 * @param loadLibraries
	 */
	private static String getLibraries(ArrayList<GmapLibrary> loadLibraries) {
		if (loadLibraries == null) {
			return "";
		}
		String s = "libraries=";
		Iterator<GmapLibrary> itr = loadLibraries.iterator();
		int i = 0;
		while (itr.hasNext()) {
			GmapLibrary ll = itr.next();
			if (ll != null) {
				if (i > 0) {
					s += ",";
				}
